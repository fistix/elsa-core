...
class RadioFieldDriver {
    constructor() {
        ...
        this.updateEditor = (activity, property, formData) => {
            const value = /*property.name;*/ formData.get(property.name).toString();
...
class FileFieldDriver {
    constructor() {
        this.displayEditor = (activity, property) => {
            const name = property.name;
            const label = property.label;
            const value = activity.state[name] || '';
            const valueJson = encodeURI(JSON.stringify(value));
            return `<wf-file-field name="${name}" label="${label}" hint="${property.hint}" value="${valueJson}"></wf-file-field>`;
        };
        ...
        this.updateEditor = (activity, property, formData) => {
            debugger
            const value = formData.get(property.name);//.toString().trim();
            
            if (value) {
                activity.state[property.name] = JSON.parse(value);
            }
            //activity.state[property.name] = base64File;
        };
    }
}
...
class DatetimeFieldDriver {
    constructor() {
        ...
        this.updateEditor = (activity, property, formData) => {
            const value = formData.get(property.name).toString().trim();
            activity.state[property.name] = value ? new Date(value) : Date.parse("");
        };
    }
}

class TriggerFieldDriver {
    constructor() {
        this.displayEditor = (activity, property) => {
            const name = property.name;
            const label = property.label;
...
class SelectDynamicFieldDriver {
    constructor() {
        this.displayEditor = (activity, property) => {
            const name = property.name;
            const label = property.label;
            ...
            const items = property.options.items || [];
            const itemsJson = encodeURI(JSON.stringify(items));
            return `<wf-selectdynamic-field name="${name}" label="${label}" hint="${property.hint}" data-items="${itemsJson}" value="${value}"></wf-selectdynamic-field>`;
        };
        this.updateEditor = (activity, property, formData) => {
            const value = formData.get(property.name).toString();
            activity.state[property.name] = value.trim();
...
class SelectMessagingFieldDriver {
    constructor() {
        this.displayEditor = (activity, property) => {
            ...
            const items = property.options.items || [];
            const itemsJson = encodeURI(JSON.stringify(items));
            return `<wf-selectmessaging-field name="${name}" label="${label}" hint="${property.hint}" stateVals="${stateValue}" data-items="${itemsJson}" value="${value}"></wf-selectmessaging-field>`;
        };
...
class RepeatFieldDriver {
    constructor() {
        this.displayEditor = (activity, property) => {
            ...
            debugger
            let jsonValue = "";
            if (value != '') jsonValue = encodeURI(JSON.stringify(value))
            const items = property.options.items || [];
            const itemsJson = encodeURI(JSON.stringify(items));
            return `<wf-repeat-field name="${name}" label="${label}" hint="${property.hint}" data-items="${itemsJson}" value="${jsonValue}"></wf-repeat-field>`;
        };
        ...
        this.updateEditor = (activity, property, formData) => {
            ...
            //else {
                const value = formData.get(property.name);
            activity.state[property.name] = value && value != "" ? JSON.parse(value) : "";
            //}
        };
    }
}
...
class DesignerHost {
    constructor(hostRef) {
        ...
        this.initFieldDrivers = () => {
            ...
            DisplayManager.addDriver('selectDynamic', new SelectDynamicFieldDriver());
